...
const ARROW_ICON = '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>';
              ...
              ${user.projects.map((p, i) => `
                <a href="/project-detail?uid=${encodeURIComponent(user.id)}&pid=${encodeURIComponent(p.id || i)}" class="mp-proj-card">
                  ...
                  <div class="mp-proj-info">
                    ...
                    <h3>${p.name}</h3>
                    <p>${p.desc || ''}</p>
                    <div class="mp-proj-view">View details ${ARROW_ICON}</div>
                  </div>
                </a>
              `).join('')}
            </div>
